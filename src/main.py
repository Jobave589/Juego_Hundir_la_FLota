...
while True:
    ...
    while vidas_usuario > 0 and vidas_maquina > 0:
        vidas_maquina = turno_usuario(
            tablero_maquina, vidas_maquina, tablero_usuario)
        print(f"Vidas restantes de la máquina: {vidas_maquina}")

        if vidas_maquina == 0:
            mensaje_lento(
                "¡ENHORABUENA! Eres un maestro de la artillería. ¡Adelante!")
            break

        vidas_usuario = turno_maquina(
            tablero_usuario, vidas_usuario, dificultad)
        print(f"Vidas restantes del usuario: {vidas_usuario}")

        if vidas_usuario == 0:
            mensaje_lento(
                "Capitán, nos estamos hundiendo. Esta vez no ha podido ser.")
            break
